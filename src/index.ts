...
app.get("/callback", async (req: Request, res: Response) => {
  try {
    ...
    // After successful authentication, set up the schedulers
    setupTweetScheduler();
...
function setupTweetScheduler() {
  ...
  logger.info("Tweet scheduler set up for 12:00 PM daily");
  return scheduler;
}
...
function setupMetricsScheduler() {
  ...
  logger.info("Metrics scheduler set up for 12:00 PM daily");
...
if (tokenData.authenticated) {
  ...
  setupTweetScheduler();
  setupMetricsScheduler();
  logger.info("Tweet and metrics schedulers initialized on startup");
} else {
  logger.info(
